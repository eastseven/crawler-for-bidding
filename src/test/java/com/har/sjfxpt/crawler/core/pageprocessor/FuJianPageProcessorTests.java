...
import com.har.sjfxpt.crawler.ggzy.provincial.FuJianPageProcessor;
...
public class FuJianPageProcessorTests {

    @Autowired
    FuJianPageProcessor fuJianPageProcessor;
    ...
    public void testGGZYFuJianAnnotation() {
        List<SourceModel> list = SourceConfigAnnotationUtils.find(fuJianPageProcessor.getClass());

        List<Request> requestList = list.parallelStream().map(SourceModel::createRequest)
                .collect(Collectors.toList());
        log.debug("requestList={}", requestList);
        Spider.create(fuJianPageProcessor)
